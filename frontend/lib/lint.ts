...
// ---------------------------------------------------------------------------
// Main lint function — runs entirely in the browser, no API call needed
// ---------------------------------------------------------------------------
...
export function lintContract(code: string): LintResult {
  ...
  if (!hasHeader) {
    errors.push(
      item('error', 1, 1, 'Missing GenLayer dependency header. First line must be # { "Depends": "py-genlayer:test" }')
    );
  }
  ...
  if (contractClassLines.length === 0) {
    errors.push(item('error', 1, 1, 'No contract class found. Define a class that extends gl.Contract'));
  }

  // ── ERROR 3: Multiple contract classes ───────────────────────────────────
  if (contractClassLines.length > 1) {
    contractClassLines.slice(1).forEach((ln) => {
      errors.push(item('error', ln, 1, 'Only one gl.Contract subclass is allowed per contract file'));
    });
  }

  // ── Contract-level checks (only if exactly one class found) ──────────────
  if (contractClassLines.length === 1) {
    const classLine = contractClassLines[0];
    const classIndent = indent(lines[classLine - 1]);

    const classBodyLines = lines
      .slice(classLine)
      .filter((l) => l.trim() === '' || indent(l) > classIndent);

    // ── ERROR 5: Missing __init__ ─────────────────────────────────────────
    const hasInit = classBodyLines.some((l) => /def\s+__init__\s*\(/.test(l));
    if (!hasInit) {
      errors.push(item('error', classLine, 1, 'Contract class is missing __init__ method'));
    }
    ...
    if (hasInit) {
      ...
      if (initIdx !== -1) {
        ...
        if (nonPassBody.length === 0 && bodyLines.length > 0) {
          info.push(
            item('info', initIdx + 2, 1, '__init__ has no state initialization. Add state variables if your contract needs persistent data')
          );
        }
      }
    }
    ...
    if (!hasPublicMethods) {
      info.push(
        item('info', classLine, 1, 'No public methods found. Add @gl.public.view or @gl.public.write methods')
      );
    }
    ...
    for (let i = classLine; i < lines.length; i++) {
      ...
      if (!/\)\s*->\s*\S/.test(defLine)) {
        warnings.push(
          item('warning', defLineNo, 1, `Method '${methodName}' is missing a return type annotation`)
        );
      }
      ...
      const methodBodyIndent = methodIndent + 4;
      ...
      for (const { lineNo: bln, text: bl } of methodBody) {
        ...
        if (/gl\.eq_principle_/.test(stripped)) methodHasEqPrinciple = true;

        const hasNonDet =
          /gl\.exec_prompt\s*\(/.test(stripped) || /gl\.nondet\b/.test(stripped);

        if (hasNonDet) {
          if (!insideNestedDef && directNonDetLine === -1) directNonDetLine = bln;
          if (insideNestedDef) nestedHasNonDet = true;
        }
      }
      ...
      if (nestedHasNonDet && !methodHasEqPrinciple) {
        warnings.push(
          item('warning', defLineNo, 1, 'Inner function with non-deterministic call found but no gl.eq_principle_* wrapper detected')
        );
      }
    }
  }
  ...
  lines.forEach((l, i) => {
    ...
    while ((match = ethAddressRe.exec(l)) !== null) {
      warnings.push(
        item('warning', i + 1, match.index + 1, `Hardcoded address detected. Use Address('0x...') for type safety`)
      );
    }
  });
